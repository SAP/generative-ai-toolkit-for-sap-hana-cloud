...
class FFTInput(BaseModel):
    ...
    """
    table_name : str = Field(description="Table (or view) that contains the input sequence data for applying FFT. If not provided, ask the user, do not guess")
    key : str = Field(description="the key of the dataset. If not provided, ask the user. Do not guess")
    data_cols : dict = Field(description="the data columns (i.e. real part and imaginary part of the sequence data) involved for FFT in a dict, " +\
    "with keys being column number type (either 'real' or 'imag') and values being column names. " +\
...
class FFT(BaseTool):
    ...
    str
        ...
        .. note::
            ...
            .. list-table::
                ...
                * - data_cols
                  - Specifies the data columns (i.e. real and imaginary part of the sequence data) involved for FFT in a dict,
                    with keys for column number type (either 'real' or 'imag') and values for column names.
                    If not provided, ask the user. Do not guess
                * - inverse
    ...
    def _run(#pylint:disable=too-many-positional-arguments
        ...
        num_type = None
        cols = []
        if all(ky not in data_cols for ky in ['real', 'imag']) or len(data_cols) > 2:
            msg = "keys of data_cols must be either 'real' or 'imag'"
            raise ValueError(msg)
        if len(data_cols) == 1:
            num_type = list(data_cols.keys())[0]
            cols.append(data_cols[num_type])
        else:
            cols.append(data_cols['real'])
            cols.append(data_cols['imag'])
