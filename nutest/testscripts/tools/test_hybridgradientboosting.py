...
from hana_ai.tools.hana_ml_tools.hybridgradientboosting_tools import (
    HGBTreeFitAndSave,
    HGBTreeLoadModelAndPredict,
    HGBTreeLoadModelAndScore
)

class TestHGBTree(unittest.TestCase):
    """Test Suite for Hybrid Gradient Boosting Tree."""
    
    def setUp(self):
        """Initialize test environment."""
        self.mock_connection = MagicMock(spec=dataframe.ConnectionContext)
        self.mock_connection.connection = MagicMock()
        self.mock_connection.pyodbc_connection = MagicMock()  # <--- Ergänzung!
        self.fit_tool = HGBTreeFitAndSave(connection_context=self.mock_connection)
        self.predict_tool = HGBTreeLoadModelAndPredict(connection_context=self.mock_connection)
        self.score_tool = HGBTreeLoadModelAndScore(connection_context=self.mock_connection)
        self.mock_table = MagicMock()
        self.mock_table.select_statement = "SELECT * FROM TEST_TABLE"
        # Mock für .select().distinct().count()
        def mock_count():
            return 5  # beliebiger Integer für Kategorie-Erkennung
    ...
    @patch('hana_ai.tools.hana_ml_tools.hybridgradientboosting_tools.HybridGradientBoostingTree')
    @patch('hana_ai.tools.hana_ml_tools.hybridgradientboosting_tools.ModelStorage')
    def test_fit_and_save(self, mock_storage_class, mock_hgb_class):
        """Test training and saving."""
        mock_model = MagicMock()
        mock_model.fit.return_value = None
        mock_model.score.return_value = 0.9  # <--- Mock gibt Float zurück!
        mock_hgb_class.return_value = mock_model
        ...
        self.assertEqual(result_dict["model_name"], "TEST_HGB_MODEL")
    ...
    def test_nan_values_error(self):
        ...
        with self.assertRaises(ValueError):
            if np.isnan(data_with_nan).any():
                raise ValueError("NaN-Werte gefunden!")
...
class TestHGBTreeAdvanced(unittest.TestCase):
    """Erweiterte Tests für Hybrid Gradient Boosting Tree."""
    ...
    @patch('hana_ai.tools.hana_ml_tools.hybridgradientboosting_tools.HybridGradientBoostingTree')
    @patch('hana_ai.tools.hana_ml_tools.hybridgradientboosting_tools.ModelStorage')
    def test_fit_with_large_feature_set(self, mock_storage_class, mock_hgb_class):
        mock_model = MagicMock()
        mock_model.fit.return_value = None
        mock_model.score.return_value = 0.8  # <--- Mock gibt Float zurück!
        mock_hgb_class.return_value = mock_model
        mock_storage = MagicMock()
        mock_storage.save_model.return_value = True
        mock_storage_class.return_value = mock_storage
    ...
    @patch('hana_ai.tools.hana_ml_tools.hybridgradientboosting_tools.HybridGradientBoostingTree')
    @patch('hana_ai.tools.hana_ml_tools.hybridgradientboosting_tools.ModelStorage')
    def test_fit_with_missing_table(self, mock_storage_class, mock_hgb_class):
